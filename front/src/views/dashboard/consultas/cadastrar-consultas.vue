	...
	<LayoutDashboard>
		<div class="grid grid-cols-1 gap-4 px-12 mt-12">
			<router-link to="/dashboard/medicos">
				<h2 class="text-xl font-semibold text-gray-600 transition duration-300 ease-in-out hover:text-gray-800">
					Voltar
				</h2>
			</router-link>
			...
			<div class="w-full bg-white rounded-lg shadow-lg max-w-[1000px] mx-auto p-10">
				<div class="grid grid-cols-1 gap-4">
					<div>
						...
						<select class="w-full select select-bordered texto-opcoes" v-model="optionSelected.fullName">
					...
					<div>
						...
						<input type="text" class="w-full rounded-full input input-bordered texto-opcoes"
					...
					<div>
						...
						<select class="w-full select select-bordered texto-opcoes" v-model="optionSelected.specialty">
					...
					<div>
						...
						<select class="w-full select select-bordered texto-opcoes" v-model="optionSelected.medicName">
					...
					<div>
						...
						<select class="w-full select select-bordered texto-opcoes" v-model="optionSelected.room">
					...
					<div class="grid grid-cols-1 gap-1">
						...
						<input type="text" placeholder="Digite o motivo da consulta"
							class="w-full rounded-full input input-bordered texto-opcoes" v-model="motivo" />
					</div>
					...
					<button class="p-1 w-1/6 text-white btn btn-active btn-neutral hover:bg-blue"
						style="background-color: #00428f" @click.prevent="handleSubmit" :disabled="isSubmitting">
					...
					<dialog id="my_modal_2" class="modal">
						<div class="grid grid-cols-1 gap-4 modal-box">
							...
							<button class="w-1/6 text-white btn btn-active btn-neutral hover:bg-blue"
								style="background-color: #00428f" onclick="my_modal_2.close()">
								Fechar
							</button>
						</div>
...
// adicione um watch para doctors
watch(doctors, (newDoctors) => {
	if (newDoctors && newDoctors.length > 0) {
		console.log('médicos carregados:', newDoctors)
	}
})
