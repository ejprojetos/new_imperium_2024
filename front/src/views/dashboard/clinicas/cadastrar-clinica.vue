    ...
    <LayoutDashboard>
        <div class="p-8">
            <h1 class="mb-6 text-4xl font-bold">
                {{ isEditMode ? 'Editar' : 'Cadastrar' }} Clínica
            </h1>
...
const formatErrorMessage = (error: any): string => {
    if (error?.errors) {
        const messages = Object.entries(error.errors).map(([field, messages]) => {
...
const submitForm = async () => {
    try {
        ...
        if (!result) {
            toast.error(
                `Erro ao ${isEditMode.value ? 'atualizar' : 'cadastrar'} clínica. Por favor, tente novamente.`
            )
            return
        }
