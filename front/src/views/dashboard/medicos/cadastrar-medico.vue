<template>
	<LayoutDashboard>
		...
		<div class="grid grid-cols-1 gap-4 ml-[25px] sm:ml-[145px] mt-[20px]">
			<h1 class="mb-6 text-[1.75rem] font-normal sm:text-[3.125rem] sm:font-montserrat">Cadastrar Médico</h1>
			<div class="rounded-xl sm:rounded-none pb-[70px] pt-[29px] sm:pt-[40px] px-[20px] sm:px-[60px] mb-4 bg-white ded-lgroun gap-x-4 sm:w-[650px]"
				...
				<form @submit.prevent="submitForm">
					...
					<div>
						...
						<input v-model="FormData.zipCode" type="text" class="px-2 py-1 rounded-2xl border w-260"
					...
					<div>
						...
						<select v-model="FormData.degree" class="w-full sm:w-[432px] px-2 py-1 border rounded-2xl">
							<option value="GraduaçãoCompleta">Graduação e Pós-Graduação completo</option>
							<option value="Mestrado">Mestrado Completo</option>
						</select>
					</div>
					<div class="mt-[20px]">
						<label class="block mt-[20px] mb-2 text-sm sm:text-lg font-montserrat br-16">Especialidade:
						</label>
						<select v-model="FormData.specialty" class="w-full sm:w-[432px] px-2 py-1 border rounded-2xl">
					...
					<div class="mt-[20px]">
						...
						<div class="flex flex-col items-start">
							<label for="file-upload"
								class="px-4 py-2 text-lg text-black bg-white rounded-full border border-gray-300 cursor-pointer sm:rounded-lg hover:bg-gray-100">Anexar</label>
							<input @change="onFileChange" type="file" id="file-upload" class="hidden" />
					...
					<div class="mt-[20px]">
						...
						<input v-model="FormData.crm" type="text" class="px-2 py-1 w-full rounded-2xl border"
							placeholder="999.999.999-99" />
					...
					<div>
						...
						<input v-model="FormData.password" type="password" class="px-2 py-1 w-full rounded-2xl border"
					...
					<dialog id=" my_modal_4" class="modal" ref="modal" @click.self="closeModal">
						...
						<div class="modal-box w-[310px] sm:w-[750px] max-w-5xl">
							<div>
								<h1 class="font-montserrat text-[1.375rem] font-semibold text-center">Médico cadastrado
									com sucesso!</h1>
							</div>

							<ModalCadastro :FormData="FormData">
								<template v-slot:header>
									<h1 class="mb-6 font-bold text-[30px] w-[640px] text-h1 font-montserrat">Paciente
										cadastrado com sucesso!</h1>

									<h2 class="mb-4 text-xl font-bold text-black font-montserrat">Identificação do
										paciente:</h2>
								</template>

								<h2 class="mt-[40px] mb-4 text-xl font-montserrat text-black font-bold">Dados
									Profissionais:</h2>

								<div class="flex flex-col">
									<label
										class="block mt-[20px] mb-2 text-lg font-montserrat br-16 font-semibold">Formação:
									</label>
									<p>
										{{ FormData.degree }}
									</p>
								...
								<div class="flex flex-col">
									<label
										class="block mt-[20px] mb-2 text-lg font-montserrat br-16 font-semibold">Especialidade:
									</label>
									<p>
										{{ FormData.specialty }}
									</p>
								...
								<div class="flex flex-col">
									<label
										class="block mt-[20px] mb-2 text-lg font-montserrat br-16 font-semibold">Documentos
										comprobatórios: </label>

									<p class="border rounded-3xl p-4 w-[160px]"
								...
								<div class="flex flex-col">
									<label class="block mt-[20px] mb-2 text-lg font-montserrat br-16 font-semibold">CRM:
									</label>
									<p>
										{{ FormData.crm }}
									</p>
